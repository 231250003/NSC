...
public class LLVMIRToRiscv {
    ...
    int phi_val_storage = Main.CONSTANTS.get("MAX_SP_OFFSET");
    ...
    public List<LLVMValueRef> reorderFunctionsWithMainFirst(LLVMModuleRef module) {
        ...
        for (LLVMValueRef func = LLVMGetFirstFunction(module);
            ...
            if ("main".equals(name)) {
                ...
                if (LLVM.LLVMGetFirstBasicBlock(func)!=null) {
                    functions.add(func);
                }
            }
        }
    ...
    public void pass_param(Map<Integer, Set<Integer>> param_conflict_graph, AsmBuilder asm) {
        ...
        Set<Integer> to_rem=new HashSet<>();
        for (Map.Entry<Integer, Set<Integer>> entry : param_conflict_graph.entrySet()) {
            param_conflict_graph.get(entry.getKey()).remove(entry.getKey());
             if(param_conflict_graph.get(entry.getKey()).size()==0){
                to_rem.add(entry.getKey());
            }
        }
        for (Integer tem:to_rem) {
            param_conflict_graph.remove(tem);
        }
    ...
    public void to_riscv() {
        ...
        for (LLVMValueRef func : all_function) {
            ...
            asm.label(funcName);
            if ("main".equals(funcName)) asm.instr("addi", "sp", "sp", "-" + Main.CONSTANTS.get("STACK_SIZE"));
...
//                                        System.out.println(10 + i);
                        ...
                        if (LLVMGetTypeKind(retType) == LLVMIntegerTypeKind) {
                            ...
                            if (LLVM.LLVMGetValueName(func).getString().equals("main")) {
                                asm.mv("x10", reg);
                                asm.instr("addi", "sp", "sp", "" +  Main.CONSTANTS.get("STACK_SIZE")); // Epilogue
                                if(!Main.is_rars){
                                    asm.instr("jal", "x1","exit");
                                }
                                else{
                                    asm.li("a7", 93);
                                    asm.instr("ecall");
        ...
        if(next_offset>=Main.CONSTANTS.get("MAX_SP_OFFSET")){
            System.out.println("MAX_SP_OFFSET reached,will cause segmentaion fault\n");
        }
        else asm.writeToFile(file_path);
    }
    ...
    public static void extractIntegerElements(LLVMValueRef value, List<Integer> elements) {
        ...
        if (LLVM.LLVMIsConstant(value) == 1) {
            if (LLVM.LLVMIsAConstantInt(value) != null) {
                ...
                LLVMTypeRef ty = LLVM.LLVMTypeOf(value);
                int kind = LLVM.LLVMGetTypeKind(ty);
                if (kind == LLVM.LLVMArrayTypeKind) {
                    // 数组类型
                    int numElements = LLVM.LLVMGetArrayLength(ty);
                    for (int i = 0; i < numElements; i++) {
                        elements.add(0);
                    }
                } else if (kind == LLVM.LLVMStructTypeKind) {
                    // 结构体类型
                    int numElements = LLVM.LLVMCountStructElementTypes(ty);
                    for (int i = 0; i < numElements; i++) {
                        elements.add(0);
                    }
                } else if (kind == LLVM.LLVMVectorTypeKind) {
                    // 向量类型
                    int numElements = LLVM.LLVMGetVectorSize(ty);
                    for (int i = 0; i < numElements; i++) {
                        elements.add(0);
                    }
                } else {
                    // 标量 zeroinitializer（比如 i32 zeroinitializer）
                    elements.add(0);
                }
