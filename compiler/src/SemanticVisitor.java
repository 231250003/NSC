...
public class SemanticVisitor extends SysYParserBaseVisitor<Void> {
    ...
    public Void visitFuncDef(SysYParser.FuncDefContext ctx) {
        ...
        if(ctx.block()!=null)visit_block(ctx.block(),params);
        return null;
    }
    ...
    private boolean has_variable_in_init_val(SysYParser.ConstInitValContext constInitValContext) {
        if(constInitValContext.constExp()!=null){
            if(constInitValContext.constExp().exp().number()!=null) return true;
            else return false;
        }
        else{
            boolean flag=true;
            for(int i=0;i<constInitValContext.constInitVal().size();i++){
                if(has_variable_in_init_val(constInitValContext.constInitVal().get(i))==false) {
                    flag=false;
                    break;
                }
            }
    ...
    public Symbol getVardef(SysYParser.VarDefContext ctx) {
        ...
        if(ctx.initVal()==null) {
            ...
            if(symbolTable.is_cur_scopeGlobal()){
                if(!has_variable_in_init_val(ctx.initVal()))      OutputHelper.printSemanticError(ErrorType.ASSIGNING_VARIABLE_TO_GLOBAL_VARIABLE,ctx.getStart().getLine());
            }
            if(!ctx.number().isEmpty()) {
                symbol.type=new ArrayType();
                ((ArrayType)symbol.type).dim= ctx.number().size();
    ...
    private boolean has_variable_in_init_val(SysYParser.InitValContext initValContext) {
        if(initValContext.exp()!=null){
            if(initValContext.exp().number()!=null) return true;
            else return false;
        }
        else{
            boolean flag=true;
            for(int i=0;i<initValContext.initVal().size();i++){
                if(has_variable_in_init_val(initValContext.initVal().get(i))==false){
                    flag=false;
                    break;
                }
            }
