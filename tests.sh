...
for sysy_file in "${FILE_LIST[@]}"; do
    ...
    make -s compile SRCFILE="$sysy_file" ARGS="-l -t"
    if [ $? -ne 0 ]; then
    ...
    if [ -f "$riscv_file" ]; then
            ...
            java -jar compiler/utils/rars.jar nc me "$riscv_file"
            riscv_return_value=$?
    ...
    find "$SYSY_DIR" -name "*.c" -exec rm -f {} \;

    find "$SYSY_DIR" -type f ! -name "*.*" -exec rm -f {} \;
